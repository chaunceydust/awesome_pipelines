...
while getopts "d:q:p:j:s:f:e:h" OPTION
do
    case $OPTION in
        d)
            dbSeqsList=$OPTARG
...
if [ "serial" = "serial" ];
then
    bash 1_blast_parallel.sh -p $PopName -j $threadCutoff -s $singleThreads -f $resultfmt -e $evalueCutoff
else
    bash 1_blast_serial.sh
fi

####
bash 2_anno_parallel.sh -p $PopName -j $threadCutoff
