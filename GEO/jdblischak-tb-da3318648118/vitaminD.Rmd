...
Plotting boxplots of genes for response to vitamin D at 48 hours for Figure S5.
...
```{r}
target_name <- c("VDR", "CYP27B1", "PIM1", "CXCL10")
...
```{r plot-function}
vitD <- ggplot(dat_fc_long,  aes(x = bact, y = log2FC, fill = bact)) +
  geom_boxplot(lwd = 1) +
...
```{r}
pdf("../figs/fig-S05-vitD.pdf", width = 8, height = 7,
    useDingbats = FALSE)
vitD
dev.off()
```
