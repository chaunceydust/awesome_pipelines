...
Example:
    bash $0 -d database.list -q query.list -j 20
EOF
}

# Default parameter
declare -i ComputerThreads=`cat /proc/cpuinfo | grep "processor" | wc -l`
declare -i threadCutoff=20
declare -i singleThreads=$ComputerThreads/$threadCutoff
declare -i resultfmt=0
evalueCutoff=1e-5

dbSeqsList="database.list"
querySeqsList="query.list"
